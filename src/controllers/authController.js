...
const signup = async (request, reply) => {
  ...
  const { name, gender, email, password, passwordConfirm } = request.body;

  const user = await User.create({ name, gender, email, password, passwordConfirm });
...
const updateMe = async (request, reply) => {
  ...
  const filteredBody = filterReqBody(request.body, 'email', 'name', 'gender');
